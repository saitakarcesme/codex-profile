...
unsafe extern "system" fn find_codex_window(hwnd: HWND, parameter: LPARAM) -> i32 {
    ...
    let resolved =
        QueryFullProcessImageNameW(process, 0, path_buffer.as_mut_ptr(), &mut path_length) != 0;
    CloseHandle(process);
    if !resolved {
        return 1;
    }
...
fn codex_window_bounds() -> Option<RECT> {
    ...
    unsafe {
        EnumWindows(
            ...
            &mut search as *mut CodexWindowSearch as LPARAM,
        );
    }
    search.bounds
}
    ...
    thread::spawn(move || loop {
        if let Some(launcher) = handle.get_webview_window("launcher") {
            if let Some(bounds) = codex_window_bounds() {
                let size = launcher
                    .outer_size()
                    .unwrap_or(tauri::PhysicalSize::new(58, 58));
                let x = bounds
                    .right
