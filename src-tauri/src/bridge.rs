...
pub const BRAND_ICON_PNG: &[u8] = include_bytes!("../../assets/brand/codex-profile-logo.png");
    ...
    fn execute(&self, args: &[&str]) -> Result<String, String> {
        ...
        if !output.status.success() {
            ...
            return Err(if message.is_empty() {
                "Codex Profile operation failed".into()
            } else {
                message
            });
        }
...
fn discover_node() -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    candidates
        .into_iter()
        .find(|path| path.is_file())
        .ok_or_else(|| "Node.js 20+ was not found".into())
}
...
pub async fn list_profiles(bridge: State<'_, CoreBridge>) -> Result<Value, String> {
    ...
    let mut value: Value =
        serde_json::from_str(&raw).map_err(|_| "Codex Profile returned invalid profile data")?;
    if let Some(profiles) = value.get_mut("profiles").and_then(Value::as_array_mut) {
        for profile in profiles {
            let avatar = profile
                .get("avatar")
                .and_then(Value::as_str)
                .and_then(avatar_data_url);
...
#[tauri::command]
pub fn brand_icon() -> String {
    format!("data:image/png;base64,{}", STANDARD.encode(BRAND_ICON_PNG))
}
