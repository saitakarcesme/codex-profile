...
The selector is a lightweight Tauri 2 shell around a framework-free TypeScript/Vite frontend. Its compact 720 x 420 borderless surface uses native WebView rendering, platform-matched transparent outer corners (8 px on Windows and 16 px on macOS), the current Codex neutral-gray palette, one bundled transparent Codex Profile logo across the title bar, launcher, tray/menu bar, taskbar, shortcuts, and application bundles, high-resolution protected avatars, centered vector controls, a custom drag region, smooth selected/hover states, and reduced-motion support. Each linked ChatGPT account dynamically supplies its own profile picture; the validated bounded image is cached privately per account, while initials remain the fallback when no picture is available. macOS treats the tray asset as a template image so it follows light/dark menu-bar contrast; dark in-app surfaces render the same alpha mask in white. There are no profile cards, embedded screenshots, Electron runtime, terminal window, or proprietary Desktop patches. A 52 px always-on-top launcher follows the public Codex window rectangle, stays out of the taskbar, hides when Codex closes, and opens the selector when clicked. Both title-bar controls hide the selector to this launcher/tray instead of leaving an inaccessible minimized window. It does not read or modify Codex renderer content. The Rust bridge strips credential paths and auth material before data reaches the webview. The previous WPF/PowerShell implementation lives under `legacy/windows` only as an unpackaged fallback when a Tauri binary cannot be found.
