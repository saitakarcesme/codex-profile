...
The 780 x 470 interface is live HTML/CSS, not a screenshot: neutral Codex-gray radial depth, low-opacity grain, slowly moving blurred luminance fields, native font stacks, 108-122 px circular avatars, a small selected badge, and a matching circular Add Account action. It has no card rectangles or dashed borders, includes hover/active/focus-visible states, adapts profile spacing/count, and honors `prefers-reduced-motion`. The title and attached 52 px launcher load the original tray icon from the locally installed Codex package at runtime; the project does not distribute a copied proprietary icon. Automated Chromium rendering verifies zero overflow at 100%, 125%, and 150% raster scales; the release WebView was separately inspected with real profile data.
