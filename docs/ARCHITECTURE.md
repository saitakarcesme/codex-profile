...
The desktop selector is Tauri 2: a Rust native shell, a framework-free TypeScript/Vite frontend, and a thin allow-listed bridge to the unchanged Node core. The bridge can list credential-free profile projections, start an isolated Add Account flow, or request a Desktop switch. Avatar files are converted to bounded data URLs in Rust. A real ChatGPT/Codex username may be stored as credential-free metadata and projected to the selector; it is never inferred from an email address. The full email, local avatar path, raw CLI payload, and every credential field are removed before the webview receives data. Long Node operations run on blocking workers rather than freezing the UI thread.
