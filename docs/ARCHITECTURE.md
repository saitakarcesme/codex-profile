...
The desktop selector is Tauri 2: a Rust native shell, a framework-free TypeScript/Vite frontend, and a thin allow-listed bridge to the unchanged Node core. The bridge can list credential-free profile projections, start an isolated Add Account flow, or request a Desktop switch. Each ChatGPT account's authenticated profile response is used to discover its own picture dynamically; validated JPEG/PNG/WebP files are capped at 5 MB, cached privately beside that account's protected snapshot, and converted to bounded data URLs in Rust. No fixed user image or remote URL is shipped in the application. A real ChatGPT/Codex username may be stored as credential-free metadata and projected to the selector; it is never inferred from an email address. When that independent username is unavailable, the selector uses the account email already exposed by Codex instead of a local label or token display name. Local avatar paths, raw CLI payloads, and every credential field are removed before the webview receives data. Long Node operations run on blocking workers rather than freezing the UI thread.
